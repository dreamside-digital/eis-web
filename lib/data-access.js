...
export async function getPageTranslations() {
  try {
    ...
    const artboxDiscover = await fetchCollection('artbox_discover')
    ...
    return {
      ...
      'artbox_discover': artboxDiscover
    }
...
export async function backfillMissingPalettes() {
  try {
    const api = directus(process.env.DIRECTUS_TOKEN)
    ...
    if (artworks.length === 0) return { processed: 0, failed: 0 }

    let processed = 0
    let failed = 0

    for (const artwork of artworks) {
      const result = await extractArtworkPalette(artwork.id)
      if (result.success) {
        processed++
      } else {
        failed++
    ...
    console.error('Backfill palettes error:', error)
    return { processed: 0, failed: 0 }
  }
}

export async function extractArtworkPalette(artworkId) {
  try {
    const api = directus(process.env.DIRECTUS_TOKEN)

    // Fetch artwork to get first image only
    const artwork = await api.request(
      readItem('artworks', artworkId, {
        fields: ['id', 'images.directus_files_id.id'],
      })
    )

    const imageId = artwork?.images?.[0]?.directus_files_id?.id
    if (!imageId) return { success: false, error: 'No images' }

    // Fetch a 200x200 thumbnail
    const imageUrl = `${process.env.NEXT_PUBLIC_DIRECTUS_URL}/assets/${imageId}?width=200&height=200&fit=cover`
    const imageResponse = await fetch(imageUrl)
    if (!imageResponse.ok) return { success: false, error: 'Failed to fetch image' }

    const imageBuffer = Buffer.from(await imageResponse.arrayBuffer())

    const sharp = (await import('sharp')).default
    const { kMeansPalette } = await import('@/lib/color-utils')
    ...
    await api.request(
      updateItem('artworks', artworkId, { color_palette: palette })
    )

    return { success: true, palette }
  } catch (error) {
    console.error('Palette extraction error:', error)
    return { success: false, error: 'Failed to extract palette' }
  }
}
