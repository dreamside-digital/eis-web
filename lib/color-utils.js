...
// Returns average Delta-E per user colour so scores are comparable across
// different palette sizes and readable on the perceptual Delta-E scale.
// A chroma penalty discourages pairing achromatic user colours (black, white,
// grey) with saturated artwork colours that share similar lightness but differ
// strongly in saturation.
const CHROMA_WEIGHT = 0.25
...
export function paletteSimilarity(palette1, palette2) {
  ...
  for (const lab1 of labs1) {
    ...
    for (let j = 0; j < labs2.length; j++) {
      ...
      const c2 = Math.sqrt(labs2[j].a ** 2 + labs2[j].b ** 2)
      const dist = deltaE(lab1, labs2[j]) + CHROMA_WEIGHT * Math.abs(c1 - c2)
      if (dist < bestDist) {
        bestDist = dist
        bestIdx = j
      }
